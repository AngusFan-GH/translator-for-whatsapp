...
class BingTranslator {
  ...
  generateTTSData(text, language, speed) {
    const lanCode = this.LAN_TO_CODE.get(language);
  ...
  constructDetectParams(text) {
    ...
    const data = `&fromLang=auto-detect&to=zh-Hans&text=${encodeURIComponent(text)}`;
  ...
  constructTranslateParams(text, from, to) {
    ...
    const translateData = `&fromLang=${this.LAN_TO_CODE.get(from)}&to=${this.LAN_TO_CODE.get(
      to,
