...
class BingTranslator {
  ...
  constructDetectParams(text) {
    let url = `ttranslatev3?isVertical=1&IG=${this.IG}&IID=${this.IID
      }.${this.count.toString()}`,
  ...
  constructTranslateParams(text, from, to) {
    let translateURL = `ttranslatev3?isVertical=1&IG=${this.IG}&IID=${this.IID
      }.${this.count.toString()}`,
  ...
  constructLookupParams(text, from, to) {
    let lookupURL = `tlookupv3?isVertical=1&IG=${this.IG}&IID=${this.IID
      }.${this.count.toString()}`,
      lookupData = `&from=${
  ...
  constructExampleParams(from, to, text, translation) {
    let exampleURL = `texamplev3?isVertical=1&IG=${this.IG}&IID=${this.IID
      }.${this.count.toString()}`,
      exampleData = `&from=${
        // Use detected language.
        from
